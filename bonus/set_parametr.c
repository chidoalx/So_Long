...
/*   Updated: 2024/02/09 12:43:48 by ael-fagr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_imag_file(t_data *arg)
{
	...
	i = 50;
	arg->grave = mlx_xpm_file_to_image(arg->mlx, "./textures/grave.xpm", &i, &i);
	if (!(arg->w) || !(arg->l) || !(arg->p_r) || !(arg->p_l)
		|| !(arg->p_u) || !(arg->p_d) || !(arg->c) || !(arg->e)
		|| !(arg->enemy) || !(arg->e_o) || !(arg->grave))
	{
		ft_putstr_fd("Error\ncheck image path", 1);
		exit (1);
	}
